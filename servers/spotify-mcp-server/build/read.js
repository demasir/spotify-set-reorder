import { z } from 'zod';
import { createSpotifyApi, formatDuration, handleSpotifyRequest, loadSpotifyConfig, spotifyFetch, } from './utils.js';
    ...
    handler: async (_args, _extra) => {
        try {
            ...
            const deviceInfo = device
                ...
                : 'Unknown device';
            const volume = device?.volume_percent !== null && device?.volume_percent !== undefined
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Error getting available devices: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
            };
        }
...
const removeUsersSavedTracks = {
    ...
    handler: async (args, _extra) => {
        ...
        try {
            ...
            const response = await fetch(`https://api.spotify.com/v1/me/library?uris=${encodeURIComponent(uris)}`, {
                method: 'DELETE',
                headers: {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Error removing tracks from Liked Songs: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
            };
        }
