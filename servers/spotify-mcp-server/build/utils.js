...
const CONFIG_FILE = process.env.SPOTIFY_CONFIG_PATH ||
...
export function loadSpotifyConfig() {
    if (!fs.existsSync(CONFIG_FILE)) {
        throw new Error(`Spotify configuration file not found at ${CONFIG_FILE}. Please create one with a clientId (clientSecret is optional — omit it to use PKCE auth).`);
    }
    try {
...
export async function spotifyFetch(endpoint, options = {}) {
    ...
    if (!config.accessToken) {
        throw new Error('No access token available. Run "npm run auth" to authenticate.');
    }
    ...
    if (!response.ok) {
        ...
        throw new Error(`Spotify API ${method} ${url} failed (${response.status}): ${errBody}`);
    }
...
export async function createSpotifyApi() {
    ...
    if (config.accessToken && config.refreshToken) {
        ...
        if (shouldRefresh) {
            ...
            try {
                ...
                throw new Error('Failed to refresh access token. Please run "npm run auth" to re-authenticate.');
            }
        }
        if (cachedSpotifyApi) {
    ...
    throw new Error('No access token available. Run the auth flow first (e.g. `npm run auth` or `/spotify-set-reorder:setup`).');
}
...
async function exchangeCodeForToken(code, config, codeVerifier) {
    ...
    if (config.clientSecret) {
        ...
        if (!codeVerifier) {
            throw new Error('PKCE auth requires a code_verifier when no clientSecret is configured');
        }
